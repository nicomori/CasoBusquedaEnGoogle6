...
public abstract class DSL {
	...
	protected boolean genericVerifyIfAnExclusiveClassAppearWithAnText(String eclusiveClass, String stringInTheElement) {
		...
		try {
			driver.findElement(By.xpath(String.format("//*[contains(@class,'%s')]//*[contains(text(),'%s')]",
					eclusiveClass, stringInTheElement))).isDisplayed();
			return true;
		} catch (Exception e) {
			driver.findElement(By.xpath(String.format("//*[contains(@class,'%s')]//*[contains(text(),'%s')]",
					eclusiveClass, stringInTheElement))).isDisplayed();
	...
	protected void genericClickByButtonContainsSomeText(String stringInTheElement) {
		System.out.println("Making a generic click in a web element with the text \"" + stringInTheElement + "\"");
		driver.findElement(By.xpath(String.format("//button[contains(text(),'%s')]", stringInTheElement))).click();
	}
	...
	protected void genericClickByContainsSomeTextAndWithSomeSpecificIndex(String stringInTheElement, String index) {
		System.out.println("Making a generic click in a web element with the text \"" + stringInTheElement + "\"");
		driver.findElement(By.xpath(String.format("(//*[contains(text(),'%s')])['%s']", stringInTheElement, index)))
				.click();
	}
	...
	/**
	 * this method change the focus of the frame
	 * 
	 * @param locator
	 */
	protected void switchToOtherFrame(By locator) {
		System.out.println("Starting to change to other frame");
		waitForAnExplicitElement(locator);
	 ...
	 * @param locator
	 */
	protected void clickJSx2AndRefreshInCaseWeNeed(By locator) {
		System.out.println("Starting click Js or refresh, wait and clickJs");
	 ...
	 *            to send
	 */
	protected void sendKeysToLocator(By locator, String stringToSend) {
	...
	protected void waitLongForAnExplicitElement(By locator) {
		System.out.println("Starting to wait Long Time for a exclusive element");
		@SuppressWarnings("unused")
	...
	/**
	 * this method wait a time defined in the parameters, for a exclusive elemente deployed
	 * 
	 * @param locator
	 * @param int with the miliseconds to wait
	 */
	protected void waitParameterTimeForAnExplicitElement(By locator, int miliseconds) {
		System.out.println("Starting to wait Long Time for a exclusive element, with this parameter time in mms: "+miliseconds);
		@SuppressWarnings("unused")
		WebElement myDynamicElement = (new WebDriverWait(driver, miliseconds))
				.until(ExpectedConditions.presenceOfElementLocated(locator));
	}
	...
	/**
	 * this method move the focus to the second tab and after close this tab and
	 * return the focus in the first one.
	 */
	...
	/**
	 * this method get all the string content in some web elements and return a
	 * list.
	 *
	 * @param locator
	 *
	...
	/**
	 * this method make scroll down in a mobile Native App until a text appear, and
	 * after all that make click in the text.
	 * 
	 * @param textToFind
